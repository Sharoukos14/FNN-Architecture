...
//H2 -> H3
double weights_h2_to_h3[H2][H3];
double der_errors_w_h2_to_h3[H2][H3];

double bias_h3[H3];
double der_erros_b_h3[H3];

double h3_total_inputs[H3];
double h3_outputs[H3];


//H3 -> Out
double weights_h3_to_out[H3][K];
double der_errors_w_h3_to_out[H3][K];
...
double* forward_pass(double *x,int d,double *y,int k){
    ...
    for(int i =0;i<H3;i++){
        double temp =0;
        for(int  j =0;j<H2;j++){
            temp += h2_outputs[j] * weights_h2_to_h3[j][i];
        }
        temp+=bias_h3[i];
        h3_total_inputs[i] = temp;
        h3_outputs[i] = func_activation(temp,ACTIVATION_TANH);
    }
    ...
    for(int i =0;i<k;i++){
        double temp =0;
        for(int  j =0;j<H3;j++){
            temp += h3_outputs[j] * weights_h3_to_out[j][i];
        }
        temp+=bias_out[i];
        outs[i] = temp;
...
void backprop(double *x, int d, double *t, int k){
    ...
    for(int i = 0;i<k;i++){
        ...
        for(int j = 0;j<H3;j++){
            der_errors_w_h3_to_out[j][i] = h3_outputs[j] * delta_out[i];
        }
    }
    ...
    for(int i = 0;i<H2;i++){
        double temp = 0;
        for(int j= 0;j<k;j++){
            temp += weights_h3_to_out[i][j]*delta_out[j];
        }

        delta_H3[i] = derivative_activation(h3_total_inputs[i],ACTIVATION_TANH)*temp;
        der_erros_b_h3[i] = delta_H3[i];

        for(int j= 0;j<H1;j++){
           der_errors_w_h1_to_h2[j][i] = h2_outputs[j]*delta_H3[i];
        }
    }
    ...
    for(int i = 0;i<H2;i++){
        double temp = 0;
        for(int j= 0;j<k;j++){
            temp += weights_h2_to_h3[i][j]*delta_H3[j];
        }
...
int main(){
    ...
    //weights H2 to H3 and biases of H3
    for(int i =0; i< H3;i++){
        bias_h3[i] = make_random_in_range();
        for(int j = 0;j<H2;j++){
            weights_h2_to_h3[j][i] = make_random_in_range();
        }
    }

    //weights H3 to out and biases of out
    for(int i =0; i< K;i++){
        bias_out[i] = make_random_in_range();
        for(int j = 0;j<H3;j++){
            weights_h3_to_out[j][i] = make_random_in_range();
        }
    }
    ...
    do{
        ...
        for(int i = 0;i<N/B;i++){
            ...
            double temp_der_w_h2_to_h3[H2][H3];
            memset(temp_der_w_h2_to_h3, 0, sizeof(temp_der_w_h2_to_h3));

            double temp_der_b_h3[H3];
            memset(temp_der_b_h3, 0, sizeof(temp_der_b_h3));

            double temp_der_w_h3_to_out[H3][K];
            memset(temp_der_w_h3_to_out, 0, sizeof(temp_der_w_h3_to_out));
            ...
            for(int j = 0;j<B;j++){
                ...
                for(int i = 0;i<H3;i++){
                    temp_der_b_h3[i] += der_erros_b_h3[i];
                    for(int j = 0;j<H2;j++){
                        temp_der_w_h2_to_h3[j][i] += der_errors_w_h2_to_h3[j][i];
                    }
                }
                for(int i = 0;i<K;i++){
                    temp_der_b_out[i] += der_errors_b_out[i];
                    for(int j = 0;j<H3;j++){
                        temp_der_w_h3_to_out[j][i] += der_errors_w_h3_to_out[j][i];
                    }
                }
            ...
            for(int i = 0;i<H3;i++){
                bias_h3[i] -= LEARNING_RATE*temp_der_b_h3[i];
                for(int  j = 0; j<H2;j++){
                    weights_h2_to_h3[j][i] -= LEARNING_RATE*temp_der_w_h2_to_h3[j][i];
                }
            }
            for(int i = 0;i<K;i++){
                bias_out[i] -= LEARNING_RATE*temp_der_b_out[i];
                for(int  j = 0; j<H3;j++){
                    weights_h3_to_out[j][i] -= LEARNING_RATE*temp_der_w_h3_to_out[j][i];
                }
            }
        ...
        if(fabs(total_error - previous_error) < MAX_ERROR && t > MIN_EPOCHS){
            printf("\nError Diff:%lf\n",fabs(total_error - previous_error));
            break;
        }
